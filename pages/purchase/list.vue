...
import { isPurchaseManager, purchaseItemSourceItemIds, purchaseItemSourceRequestIds, PURCHASE_REQUEST_STATUS, refreshPurchaseRequestStatus } from '@/utils/purchase.js'
...
export default {
	...
	methods: {
		...
		async removePreOrder(order) {
			...
			items.forEach((item) => {
				purchaseItemSourceItemIds(item).forEach((id) => {
			...
			const requestIds = [...new Set(items.reduce((ids, item) => ids.concat(purchaseItemSourceRequestIds(item)), []))]
